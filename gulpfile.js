...
const buildStandardsLib = () => {

    const builtScriptComment = "/** This file is automatically generated. Do not edit it!**/\n";
    const standardsDefDir = path.join(__dirname, "sources", "standards");

    const standardsDirs = fs.readdirSync(standardsDefDir).filter(entry => entry[0] !== ".");

    const standardsInfo = standardsDirs.map(buildLib.infoForStandard);
    const standardsFeatures = standardsDirs.map(stdDir => {
        return JSON.parse(fs.readFileSync(path.join(standardsDefDir, stdDir, "features.json"), utf8Enc));
    });

    const combinedStandards = standardsInfo.reduce((collection, info, index) => {
        const matchingFeatures = standardsFeatures[index];
        collection[info.id] = {
            info: info,
            methods: matchingFeatures.methods,
            properties: matchingFeatures.properties,
        };
        return collection;
    }, Object.create(null));
...
const compileVueComponents = () => {
    ...
    fs.readdirSync(vueTemplatesSourcePath).forEach(filepath => {
        ...
        const compiledRenderFunction = `/* This file is derived from sources/vue/${filepath}. */
