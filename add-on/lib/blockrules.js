...
(function () {
     ...
     *   Whether access to the DOM of child frames should be blocked.
     *
     * @return {BlockRule}
     *   A block rule object, configured to block the given standards on
     *   domains matching the match pattern.
     */
    const init = (matchPattern, standardIds, blockedFeatures, blockCrossFrame = false) => {
        let localBlockCrossFrame = blockCrossFrame;
        ...
        const getBlockCrossFrame = () => localBlockCrossFrame;

        const setBlockCrossFrame = newBlockCrossFrame => {
            localBlockCrossFrame = newBlockCrossFrame;
        };
    ...
    const fromData = object => {
        if (object.p === undefined ||
                ...
                object.f === undefined ||
                object.b === undefined) {
            throw `'Data is not a valid BlockRule: expected to find "p", "s", "f" and "b" properties.`;
        }
        ...
        return init(object.p, object.s, object.f, object.b);
    };
